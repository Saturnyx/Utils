...
async def clear_messages(ctx, amount: int = 10):
    ...
    try:
        ...
        logger.info(f"Cleared {len(deleted) - 1} messages in {ctx.channel.name} by {ctx.author}")
...
async def clear_all_messages(ctx):
    ...
    await ctx.send("⚠️ This will delete ALL messages in this channel. Type `confirm` within 10 seconds to proceed.")
    ...
    try:
        ...
        logger.info(f"Cleared all {deleted} messages in {ctx.channel.name} by {ctx.author}")
    except discord.Forbidden:
        await ctx.send("❌ I don't have permission to delete messages in this channel.")
    except discord.HTTPException as e:
        await ctx.send(f"❌ An error occurred: {e}")
...
async def clear_user_messages(ctx, user: discord.Member, amount: int = 10):
    ...
    try:
        ...
        logger.info(f"Cleared {len(deleted)} messages from {user} in {ctx.channel.name} by {ctx.author}")
    except discord.Forbidden:
        await ctx.send("❌ I don't have permission to delete messages in this channel.")
    except discord.HTTPException as e:
        await ctx.send(f"❌ An error occurred: {e}")
...
async def clear_old_messages(ctx, days: int = 7):
    ...
    try:
        ...
        await ctx.send(f"✅ Deleted {len(deleted)} messages older than {days} days.", delete_after=5)
        logger.info(f"Cleared {len(deleted)} old messages in {ctx.channel.name} by {ctx.author}")
    except discord.Forbidden:
        await ctx.send("❌ I don't have permission to delete messages in this channel.")
    except discord.HTTPException as e:
        await ctx.send(f"❌ An error occurred: {e}")
...
async def setup_auto_cleanup(ctx, channel: discord.TextChannel, days: int = 7):
    ...
    await ctx.send(f"✅ Auto cleanup enabled for {channel.mention}. Messages older than {days} days will be automatically deleted.")
...
async def auto_cleanup():
    ...
    for channel_id, settings in config['auto_cleanup'].items():
        try:
            ...
            if deleted:
                logger.info(f"Auto cleanup: Deleted {len(deleted)} messages from #{channel.name}")
